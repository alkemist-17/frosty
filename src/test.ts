...
const store = createStore<Data>({ nested: null, isData: true } as unknown as Data);
...
store.update({ nested: { isNested: false, description: "A new nested state with null innerData", innerData: null as unknown as Inner } });
...
store
    .update( { isData: false })
    .update( { nested: { isNested: true, description: 'Inner data is an empty object now', innerData: {} as Inner }})
    .update( { isData: true })
...
const s1 = store.subscribe((state: Data) => {
    console.log('Subscription 1', state.isData);
});

const s2 = store.subscribe((state: Data) => {
    console.log('Subscription 2', state.isData);
});

const s3 = store.subscribe((state: Data) => {
